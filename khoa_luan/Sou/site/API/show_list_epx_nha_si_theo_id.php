<?php 
	...
	$query = "SELECT trinh_do_tay_nghe.id_nha_si,nha_si.ho_ten_nha_si,trinh_do_tay_nghe.id_dich_vu,dich_vu.ten_dich_vu,trinh_do_tay_nghe.kinh_nghiem from trinh_do_tay_nghe inner join nha_si on nha_si.id_nha_si=trinh_do_tay_nghe.id_nha_si inner join dich_vu on dich_vu.id_dich_vu= trinh_do_tay_nghe.id_dich_vu where trinh_do_tay_nghe.id_nha_si='$id_nha_si'";
	$data = mysqli_query($connect,$query);
	$arraydata = array();
			while ($row = mysqli_fetch_assoc($data)) {
			array_push($arraydata, new Lists($row['id_nha_si'],$row['ho_ten_nha_si'],$row['id_dich_vu'],$row['ten_dich_vu'],$row['kinh_nghiem']));
					}
		echo json_encode($arraydata,256);
class Lists
	{
		function __construct($id_nha_si,$ho_ten_nha_si,$id_dich_vu,$ten_dich_vu,$kinh_nghiem)
