    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        input_vande.setOnCheckedChangeListener { group, checkedId ->
            if(checkedId == R.id.benhly_nhasi){
                clearListVideo()
                var url = "http://192.168.1.81:80/khoa_luan/API/Branch_NhaSi/api_show_nhasi_nhomdv.php?key_nhom_dich_vu=2"
                Getdata().execute(url)
                initAdapter()
                initRecyclerView()
                ...
                clearListVideo()
                var url = "http://192.168.1.81:80/khoa_luan/API/Branch_NhaSi/api_show_nhasi_nhomdv.php?key_nhom_dich_vu=1"
                Getdata().execute(url)
                initAdapter()
                initRecyclerView()
